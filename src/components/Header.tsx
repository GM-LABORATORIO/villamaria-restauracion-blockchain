...
export default function Header() {
  ...
  return (
    <header className="relative z-50 border-b border-white/10 bg-transparent backdrop-blur-md sticky top-0">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 h-20 sm:h-24 flex items-center justify-between gap-4">

        {/* Left — Institutional Brand & Allies Logos */}
        <div className="flex items-center gap-4 xl:gap-6 shrink-0">
          <Link href="/" className="flex items-center gap-3 sm:gap-4">
            <div className="relative w-10 h-10 sm:w-14 sm:h-14 rounded-2xl overflow-hidden bg-white/5 border border-white/15 p-1 flex items-center justify-center shadow-lg">
              <Image
            ...
            <div className="flex flex-col">
              <span className="text-sm sm:text-lg font-black font-mono tracking-wider text-white">
                MASPROGRESO
              </span>
              <span className="text-[9px] sm:text-[11px] text-slate-300 font-mono tracking-tight leading-tight">
                Empresa de desarrollo Territorial
              </span>
            </div>
          </Link>

          {/* Allies Badge Group — Refined Scale */}
          <div className="hidden lg:flex items-center gap-3 pl-4 xl:pl-6 border-l border-white/15" title="Entidades aliadas del proyecto">
            <div className="bg-white/5 border border-white/15 rounded-xl px-3 py-1.5 flex items-center gap-3.5 backdrop-blur-md shadow-lg">
              <Image
        ...
        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center gap-3.5 xl:gap-5 text-xs xl:text-sm font-semibold text-slate-200 shrink-0 font-sans">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
          ...
          <a
            ...
            download
            className="bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/40 px-3 py-1.5 rounded-xl text-xs font-mono font-bold text-emerald-300 transition-all flex items-center gap-1.5 whitespace-nowrap shadow-md ml-1"
          >
